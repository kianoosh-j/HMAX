	...
	for(list<list<Picture> >::iterator iter1 = input->pictures->begin(); iter1 != input->pictures->end(); iter1++){
		...
		for(list<Picture>::iterator iter2 = iter1->begin(); iter2 != iter1->end(); iter2++){
			...
			if( count % NumOfThreads == input->threadID ){
				...
				Picture distance;
				float min;
				for(list<list< Patch  > > ::iterator it1 = input->patches->begin();it1 != input->patches->end(); it1++){

					for(list< Patch >::iterator it2 = (*it1).begin(); it2 != (*it1).end(); it2++){
						...
						for(list<list<Picture> >::iterator PicIter = input->C1_result->begin(); PicIter != input->C1_result->end(); PicIter++ ){
							...
							window_patch_distance(*PicIter, it2->patches_in_different_orientations , distance);
...
int run_c2_for_all_pictures ( list<list<Picture> >&pictures , list<list< Patch > >&patches , vector<vector<vector<float> > > &C2_result){
