...
//int window_patch_distance(list<Picture> &pictures, Patch  & patches,Picture  &distance){
...
// it returns 0 on success othrewize -1
int convert_c2_results_format_from___vector_vector_vecotr_float___TO___2Dfloat_array ( list<list<Picture> > &pictures , vector<vector<vector<float> > > &C2_results , float ** &C2_results_fl ){
	...
	// it includes sized of pictures in each of 4 types
	int *sizes = new int [ pictures.size() ];

	// total number of pictures
	int total_pictures=0;

	// obtain number of pictures in each type and also total pictures
	...
	// allocation of C2 result first dimention according to total pictures
	C2_results_fl = new float * [ total_pictures ];


	// converting from source format to destination format
	...
	for ( iter_vvfloat=C2_results.begin() ; iter_vvfloat != C2_results.end() ; iter_vvfloat++){
		for ( iter_vfloat=iter_vvfloat->begin() ; iter_vfloat!=iter_vvfloat->end() ; iter_vfloat++){
			C2_results_fl[temp_index] = new float [iter_vfloat->size()];
			int temp_index2=0;
...
void print_C2_results ( vector<vector<vector<float> > > &C2_results ){
	...
	vector<string> folder_types(4);
	folder_types[0] = "train_pos";
	folder_types[1] = "train_neg";
	folder_types[2] = "test_pos";
	folder_types[3] = "test_neg";

	cout<<"@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ C2 Results\n\n";
	for ( int i=0 ; i<C2_results.size() ; i++ ){

		cout<<"\n pictures of : "<<folder_types[i]<<endl;
		for ( int j=0 ; j<C2_results[i].size() ; j++ ){

			cout<<" picture "<<j<<endl;
			for ( int k=0 ; k < C2_results[i][j].size() ; k++ ){

				cout<<setprecision(12)<< C2_results[i][j][k]<<endl;
			}
		}
	}
	cout<<"@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ C2 Results\n\n";
