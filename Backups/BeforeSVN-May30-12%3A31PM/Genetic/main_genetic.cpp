#include "../Hmax/headers/headers.h"
#include "../Hmax/headers/file_processing.h"
#include "../Hmax/headers/C1.h"
#include "../Hmax/headers/svm.h"
...
int main( int argn , char *argv[]){
	...
	for(list<string>::iterator iter = c2_files.begin(); iter != c2_files.end(); iter++ ){
		...
		if(fin.fail()==1)
		{
			cerr<<"error : in reading C2_results files an error has occured !!  "<<endl;
			return -1;
		}
...
float Objective2( GAGenome& g){
	...
	for(int i=sizes[0]+sizes[1]; i < sizes[0]+sizes[1]+sizes[2]+sizes[3] ; i++){
		result_selected[i] = new float[NumOfSelectedPatches];
