...
void config_parameter_c1(vector<int> &s1space,vector<int> &c1scaleSS,int rotations){
	s1space[0]=10;
	c1scaleSS[0]=11;
	c1scaleSS[1]=13;
	rotations = 4;
}
int extract_patches(list<Picture>&pictures, list<list<list<Picture> > > &c1patches , int ntrainpos,list<int >patch_sizes , int number_of_patches){
  //input :
	...
	int i=0;
	int rotations;
	config_parameter_c1(s1space,c1scaleSS,rotations);

	//config gabor for C1 :
	init_gabor_C1(gabor_filter_sizes,gabor_filter_rotations,div);	
	...
	{
		Picture random_pic;
		rand_picture(pictures,random_pic);

		if(C1_Function(random_pic,S1_Pictures,C1_Pictures,gabor_filters,s1space,c1scaleSS,2)!=0)
		{
		...
		//match pic_number to patches

		int number_rand_pic = random_pic.NumberInDirectory;
		match_number_to_patches(C1_Pictures,number_rand_pic);
		
		//config patches
		config_patches(c1patches,C1_Pictures,patch_sizes,rotations);
...
void config_patches(list<list<list<Picture> > > &c1patches, list<list<Picture> > &C1_Pictures ,list<int>patch_sizes,int rotations){
